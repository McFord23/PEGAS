...
public class FlyingPlayer : PlayerBase2D
{
    [Header("Flying")]
    ...
    protected override void FixedUpdate()
    {
        ...
        switch (moveState)
        {
            ...
            case MoveState.Stunned:
                rigidbody2D.linearDamping = landed ? 3f : 0.3f;
                break;
    ...
    private void RotateInput()
    {
        ...
        {
            case MoveState.Idle or MoveState.Run:
                rigidbody2D.AddTorque(rotateInput * Speed / rigidbody2D.mass);
                break;
            
            case MoveState.Flap or MoveState.FreeFall:
                rigidbody2D.AddTorque(rotateInput);
                break;
        }
    }
    ...
    private void Run(float torque)
    {
        ...
        if (landed) rigidbody2D.AddForce(transform.right * (gear * torque));
    }

    private void RidePhysics()
    {
        var friction = (landed) ? -0.3f : 0f;
        if (moveState == MoveState.Idle) friction = -rigidbody2D.mass;

        var velocity = rigidbody2D.linearVelocity;
        var drag = 0.021f * velocity.sqrMagnitude;
        var dragDirection = -velocity.normalized;

        rigidbody2D.AddForce(dragDirection * drag + velocity * friction);
    }
    ...
    private void Flap(float flapForce)
    {
        ...
        rigidbody2D.AddForce(transform.right * (gear * flapForce));
    }
    ...
    private void FlyPhysics()
    {
        ...
        rigidbody2D.AddForce(liftDirection * lift + dragDirection * drag);
    }

    private void TryFlip()
    {
        if (rigidbody2D.linearVelocityX > 0.01f && transform.localScale.y < 0)
        {
            transform.localScale = new Vector3(1, 1, 1);
        }
        else if (rigidbody2D.linearVelocityX < -0.01f && transform.localScale.y > 0)
        {
            transform.localScale = new Vector3(1, -1, 1);
        }
    }
    ...
    private void SetFlyMode(bool fly)
    {
        ...
        {
            FreeFall();
            rigidbody2D.AddForce(Vector2.up * takeoffForce);
        }
        else
        {
